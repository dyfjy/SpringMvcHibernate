...
public class GenerateJavaFile {
	...
	public static void demo() {
		...
		root.put("packageName", "com.alexgaoyh.test");// 包路径
		root.put("className", "Test");// 类名称
		root.put("springName", "test");// 类名称的首字母小写
		root.put("author", "dyf");// 作者
		root.put("tableName", "test_entity");// 表名称
		root.put("entityNameCN", "测试");// 类描述
	...
	public static void entity(String workDir, Map<String, Object> input)
			throws Exception {
	...
	public static void action(String workDir, Map<String, Object> input)
			throws Exception {
		String fileName = workDir + "/src/main/java/"
				+ input.get("packageName").toString().replaceAll("\\.", "/")
				+ "/action/" + input.get("className").toString()
				+ "Action.java";
		File myFile = new File(fileName);
		myFile.getParentFile().mkdirs();
		myFile.createNewFile();
